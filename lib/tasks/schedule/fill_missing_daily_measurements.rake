  ...
  namespace :airly do
    ...
    task fill_missing_daily_measurements: :environment do
      ...
      Location.all.each do |location|
        ...
        logger.info("Filling missing daily measurements for location #{location.name} (ID: #{location.id})
          task was performed. Number of added measurements: #{number_of_added_measurements}.")
      end
      puts 'Done'
    end
  end
